...
class ModelToolUpload extends Model {
	...
	public function addUploadCustomerID($name, $filename, $customer_id, $user_id) {
		$code = sha1(uniqid(mt_rand(), true));

		$this->db->query("INSERT INTO `" . DB_PREFIX . "upload` SET `name` = '" . $this->db->escape($name) . "', `filename` = '" . $this->db->escape($filename) . "', `code` = '" . $this->db->escape($code)
		 . "', `date_added` = NOW(), `customer_id` = '" . $this->db->escape($customer_id) . "', `user_id` = '" . $this->db->escape($user_id) ."'");
	...
	public function getUploadByCustomerId($customer_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "upload WHERE customer_id = '" . $this->db->escape($customer_id) . "'");
