...
class ControllerCustomerCustomer extends Controller {
	...
	public function import() {
		...
		$data['imports'] = array();
		$customer_id = $this->request->get['customer_id'];
		$results = $this->model_customer_customer->getCsvData($customer_id);
		
		$import_total = count($results);
		$counter = 0;
		for($i = ($page-1)*10; $i < $import_total; $i++) {
			if($counter<10){
				$data['imports'][] = array(
					'ean'    => $results[$i][0],
					'price'    => $results[$i][1],
					'date_added' => date("Y-m-d H:i:s", strtotime($results[$i][2])),
					'edit' => $this->url->link('customer/customer/editImport', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $customer_id . '&index=' . (($page-1)*10+$counter) . '&ean=' . $results[$i][0] . '&price=' . $results[$i][1] , true),
					'delete' => $this->url->link('customer/customer/importLineDelete', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $customer_id . '&index=' . (($page-1)*10+$counter), true),
					'product' => (isset($results[$i]['product_id'])) ? $this->url->link('catalog/product/edit', 'user_token='. $this->session->data['user_token'] . '&product_id=' .$results[$i]['product_id'], true) : null,
					'product_name' => (isset($results[$i]['name'])) ? $results[$i]['name'] : null,
				);
				$counter++;
			}
		}
		...
		$date_added = $this->model_customer_customer->getCsvDateAdded($customer_id);

		if(isset($date_added['username']) && isset($date_added['date_added'])){
			$data['username'] = $date_added['username'];
			$data['date_added'] = $date_added['date_added'];
		}
		$pagination = new Pagination();
		...
		$data['results'] = sprintf($this->language->get('text_pagination'), ($import_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($import_total - 10)) ? $import_total : ((($page - 1) * 10) + 10), $import_total, ceil($import_total / 10));

		$this->response->setOutput($this->load->view('customer/customer_import', $data));
	}

	public function importLineDelete(){
		$this->load->language('customer/customer');
		...
		if (!$this->user->hasPermission('modify', 'customer/customer')) {
			...
			$this->model_customer_customer->deleteCsvLine($this->request->get['customer_id'], $this->request->get['index']);
			$json['success'] = $this->language->get('text_success');
		}
		$this->getForm();
	}
	...
	public function editImport() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateImportForm()) {

			$this->model_customer_customer->editCsvLine($this->request->get['customer_id'],  $this->request->get['index'], $this->request->post);
			...
			$this->response->redirect($this->url->link('customer/customer/edit', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, true));
	...
	protected function getImportForm() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_home'),
			'href' => $this->url->link('customer/dashboard', 'user_token=' . $this->session->data['user_token'], true)
		);
		...
		if (!isset($this->request->get['customer_id'])) {
			$data['action_import'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action_import'] = $this->url->link('customer/customer/editImport', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $this->request->get['customer_id']
			. '&index=' . $this->request->get['index'] . '&ean=' . $this->request->get['ean'] . '&price=' . $this->request->get['price'], true);
		}

		//$data['cancel'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['cancel'] = $this->url->link('customer/customer/edit', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, true);
		...
		if (isset($this->request->get['ean'])) {
			$data['ean'] = $this->request->get['ean'];
		} else {
			$data['ean'] = '';
		}
		...
		$this->response->setOutput($this->load->view('customer/customer_import_form', $data));
	}

	protected function validateImportForm() {
		if (!$this->user->hasPermission('modify', 'customer/customer')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}

		if ((utf8_strlen($this->request->post['ean']) < 1) || (utf8_strlen(trim($this->request->post['ean'])) > 32)) {
